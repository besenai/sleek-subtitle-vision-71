...
import { presetThemes, fontFamilies, LANGUAGES } from "./subtitleListConstants";
...
const SubtitleSettingsPopover: React.FC<SubtitleSettingsPopoverProps> = ({
  ...
  apiKey, setApiKey, subtitleLang, setSubtitleLang, nativeLang, setNativeLang,
}) => {
  return (
    <Popover>
        ...
        <div>
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2 items-center">
              <Button size="sm" variant="outline" aria-label="Giảm" onClick={() => setFontSize(f => Math.max(12, Number(f) - 2))}>-</Button>
              <span className="text-sm px-2">{fontSize}px</span>
              <Button size="sm" variant="outline" aria-label="Tăng" onClick={() => setFontSize(f => Math.min(40, Number(f) + 2))}>+</Button>
            </div>
          </div>
          <div className="mb-3">
